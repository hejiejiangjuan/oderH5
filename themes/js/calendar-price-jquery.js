...
(function ($) {
  ...
  fn.createCalendar = function () {
    ...
    // 上一月按钮
    if (showPrevBtn)
      html += '       <a class="prev-month" title="上一月"><i class="icon-rewind"></i></a>';
    // 标题
    html += '		<div class="calendar-month-title">' + formatDate(this.month, 'yyyy年MM月') + '</div>';
    // 下一月按钮
    if (showNextBtn)
      html += '       <a class="next-month" title="下一月"><i class="icon-forward"></i></a>';
  ...
  fn.renderDataToTalbe = function () {
    ...
    this.calendar.find('.valid-hook').each(function () {
      ...
      if (dayData) {
        //首先判断是否禁用
        if (dayData.disable == 0) {
          html = '<p><span class="fool_room">不可用</span></p>'
        } else {
          //判断是否有剩余房间
          if (dayData.room_num <= 0) {
            html = '<p><span class="fool_room">满房</span></p>'
          } else {
            var con = dayData.room_num + "间";
            //判断是否加价
            if (dayData.add_price == 1) {
              con += "<span class='fool_room'></br>加价" + dayData.price +"</span>"
            } else if (dayData.add_price == 2) {
            	con += "<span class='fool_room'></br>减价" + dayData.price +"</span>"
            }
            html = '<p>'+ con +'</p>'
          }
      ...
      //   if (content) {
      //     var con1 = content.substring(0, content.indexOf('{'));
      //     var con2 = content.substring(content.indexOf('{'), content.length);
