var app = new Vue({
  ...
  methods: {
    getHotelList() {
      var url = 'http://' + location.host + '/hotel/getHotelList';
      var that = this;
      $.ajax({
        url: url,
        data: {
    ...
    //获取酒店日历
    getCalendarDetail() {
      var url = 'http://' + location.host + '/hotel/getCalendarDetail';
      var that = this;
      $.ajax({
        url: url,
        data: {
    ...
    createDatePick() {
      ...
      zxCalendar.$on('valid-day', function (day, data, next) {
        ...
        location.href = 'housetype.html?date='+day+"&order_no="+that.order_no+"&order_type=1&hotel_id="+that.hotel_id+"&hotel_name="+hotel_name+
